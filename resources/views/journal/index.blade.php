@extends('layouts.page')

@section('title', 'Jurnal — SMILJAN SOUTHSPARE')
@section('meta-description', 'Jurnal dari ruang — catatan tentang kopi, orang dan arsitektur dari Smiljan Southspare di Jakarta Selatan.')

@section('content')
    <section class="pt-32 lg:pt-44">
        <div class="wrap">
            <div class="flex items-baseline justify-between font-mono text-[0.62rem] uppercase tracking-[0.26em]">
                <span class="text-olive" data-reveal>{{ config('site.journal.index') }} / {{ config('site.journal.label') }}</span>
                <span class="hidden sm:inline" data-reveal>{{ config('site.journal.en') }}</span>
            </div>

            <div class="mt-14 grid gap-8 lg:mt-20 lg:grid-cols-12 lg:items-end">
                <h1 class="font-display text-[clamp(2.6rem,9vw,7.5rem)] leading-[0.9] tracking-[-0.01em] lg:col-span-8">
                    @foreach (config('site.journal.title') as $index => $line)
                        <span class="hero-line block whitespace-nowrap" style="--d: {{ 0.08 + $index * 0.11 }}s">{{ $line }}</span>
                    @endforeach
                </h1>
                <p class="max-w-sm text-[1.02rem] leading-relaxed text-ink/75 lg:col-span-4" data-reveal style="--d: 0.4s">
                    Catatan dari dalam ruang — kopi, orang, dan arsitektur yang kami tinggali.
                </p>
            </div>
        </div>
    </section>

    <section class="pb-28 lg:pb-44">
        <div class="wrap">
            <div class="mt-16 grid gap-16 border-t pt-12 hairline lg:mt-24 lg:grid-cols-12 lg:gap-x-10">
                @foreach ($articles as $index => $article)
                    @php
                        $featured = $index === 0;
                        $wide = $featured ? 'lg:col-span-7' : 'lg:col-span-5';
                        $offset = $index === 1 ? 'lg:mt-32' : '';
                        $titleSize = $featured ? 'text-[clamp(2rem,4.5vw,3.8rem)]' : 'text-[clamp(1.5rem,3vw,2.4rem)]';
                    @endphp
                    <article class="{{ $wide }} {{ $offset }}" data-reveal>
                        <a href="{{ route('journal.show', $article['slug']) }}" class="group block" aria-label="{{ $article['title'] }}">
                            <figure class="img-frame aspect-[4/3]">
                                <img src="{{ asset($article['src']) }}" alt="{{ $article['alt'] }}" width="1000" height="750" loading="lazy" decoding="async">
                            </figure>
                            <figcaption class="mt-4 flex items-baseline justify-between font-mono text-[0.6rem] uppercase tracking-[0.22em] text-olive">
                                <span>{{ $article['category'] }}</span>
                                <span>{{ \Carbon\CarbonImmutable::parse($article['date'])->locale('id')->translatedFormat('d M Y') }} · {{ $article['meta'] }}</span>
                            </figcaption>
                            <h2 class="mt-3 font-display {{ $titleSize }} leading-[1.02] transition-colors duration-500 group-hover:text-wood">
                                {{ $article['title'] }}
                            </h2>
                            <p class="mt-3 max-w-md text-[0.92rem] leading-relaxed text-ink/70">
                                {{ $article['excerpt'] }}
                                <span class="inline-block translate-y-[2px] font-mono text-[0.6rem] uppercase tracking-[0.2em] text-wood transition-transform duration-500 group-hover:translate-x-2">→</span>
                            </p>
                        </a>
                    </article>
                @endforeach
            </div>
        </div>
    </section>
@endsection
