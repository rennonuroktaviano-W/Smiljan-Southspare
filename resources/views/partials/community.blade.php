<section class="bg-paper" id="komunitas">
    <div class="wrap py-28 lg:py-44">
        <div class="flex items-baseline justify-between font-mono text-[0.62rem] uppercase tracking-[0.26em]">
            <span class="text-olive" data-reveal>{{ siteSetting('community', 'index') }} / {{ siteSetting('community', 'label') }}</span>
            <span class="hidden sm:inline" data-reveal>{{ siteSetting('community', 'en') }}</span>
        </div>

        <h2 class="mt-14 max-w-4xl font-display text-[clamp(2.5rem,8vw,7rem)] leading-[0.92] tracking-[-0.01em] lg:mt-20">
            @foreach (siteSetting('community.title') as $index => $line)
                <span class="reveal block" data-reveal style="--d: {{ $index * 0.1 }}s">{{ $line }}</span>
            @endforeach
        </h2>
        <p class="mt-8 max-w-md text-[1.05rem] leading-relaxed text-ink/75" data-reveal>{{ siteSetting('community', 'copy') }}</p>
