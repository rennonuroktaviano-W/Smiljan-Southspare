<section id="ruang">
    <div class="wrap py-28 lg:py-44">
        <div class="flex items-baseline justify-between font-mono text-[0.62rem] uppercase tracking-[0.26em]">
            <span class="text-olive" data-reveal>{{ config('site.space.index') }} / {{ config('site.space.label') }}</span>
            <span class="hidden sm:inline" data-reveal>{{ config('site.space.en') }}</span>
        </div>
        ...
        <div class="mt-12 grid gap-8 lg:mt-16 lg:grid-cols-12 lg:items-start">
            <div class="lg:col-span-8">
                <h2 class="font-display text-[clamp(2.4rem,7vw,6.2rem)] leading-[0.92] tracking-[-0.01em]">
                    @foreach (config('site.space.title') as $index => $line)
                        <span class="reveal block" data-reveal style="--d: {{ $index * 0.1 }}s">{{ $line }}</span>
                    @endforeach
                </h2>
                <p class="mt-8 max-w-md text-[1rem] leading-relaxed text-ink/75" data-reveal>{{ config('site.space.copy') }}</p>
            </div>
            <div class="lg:col-span-4 lg:mt-2">
                <p class="max-w-xs text-[0.95rem] leading-relaxed text-olive" data-reveal>
                    {{ config('site.space.items.0.caption') }} — {{ config('site.space.items.1.caption') }} — {{ config('site.space.items.2.caption') }} — {{ config('site.space.items.3.caption') }}
                </p>
            </div>
        </div>
